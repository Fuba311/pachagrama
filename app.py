...
def handle_file_upload(contents, filename):
    if contents is not None:
        ...
        df.rename(columns={
            ...
            'Indique la fecha del registro': 'Fecha', 
            '_id': 'ID',
        }, inplace=True)
        ...
        table_exists = inspector.has_table('table_clima22')

        if table_exists:
            existing_ids = pd.read_sql_table('table_clima22', engine, columns=['ID'])['ID'].tolist()
            df = df[~df['ID'].isin(existing_ids)]

        if not df.empty:
            df.to_sql('table_clima22', engine, if_exists='append', index=False)
...
def update_comunidad_dropdown(_):
    with engine.connect() as conn:
        ...
        df = pd.read_sql('SELECT DISTINCT "Comunidad" FROM table_clima22 ORDER BY "Comunidad"', conn)
...
def update_year_dropdown(selected_comunidad, _):
    if selected_comunidad:
        with engine.connect() as conn:
            query = f"""
            SELECT DISTINCT "Año" FROM table_clima22
...
def update_informant_dropdown(selected_comunidad, selected_year, selected_month, _):
    ...
    if selected_comunidad and selected_year and selected_month:
        with engine.connect() as conn:
            query = f"""
            SELECT DISTINCT "Informante" FROM table_clima22
...
@app.callback(
    Output('crop-discrepancies-table', 'children'),
    [Input('comunidad-dropdown', 'value'),
     Input('month-dropdown', 'value'),
     Input('year-dropdown', 'value'),
     Input('labors-toggle', 'value'),
...
def update_crop_discrepancies_table(selected_comunidad, selected_month, selected_year, show_labors, _):
    if 'show' not in show_labors or not selected_comunidad or not selected_month or not selected_year:
        return None

    with engine.connect() as conn:
        query = f"""
        SELECT "Fecha", "Informante", "Estado del maíz", "Estado del frijol"
        FROM table_clima22
        WHERE "Comunidad" = '{selected_comunidad}' AND "Mes" = '{selected_month}' AND "Año" = '{selected_year}'
        ORDER BY "Fecha" ASC;
        """
    ...
    crop_colors = {
        'Maíz': 'rgb(200, 220, 255)',  # Light blue
        'Frijol': 'rgb(150, 180, 255)'  # Blue
    }

    for crop in ['Maíz', 'Frijol']:
        crop_column = f'Estado del {crop.lower()}'
        crop_df = df.groupby('Fecha')[['Informante', crop_column]].apply(lambda x: x[crop_column].unique().tolist()).reset_index()
        crop_df.columns = ['Fecha', crop_column]
        crop_df['Multiple_Responses'] = crop_df[crop_column].apply(lambda x: len(x) > 1)
        discrepancy_days = crop_df[crop_df['Multiple_Responses']]['Fecha'].tolist()

        for day in discrepancy_days:
            day_df = df[(df['Fecha'] == day) & (df[crop_column].notna())]
            informants_info = '\n'.join([f"• {informant} (Respuesta: {response if pd.notna(response) else 'Nada'})" for informant, response in day_df[['Informante', crop_column]].values])
    ...
    return html.Div([
        html.H4(f"Días con Discrepancias en Estado del Maíz y Frijol en {selected_comunidad} - {selected_month}/{selected_year}", style={'margin-bottom': '60px'}),
        table
    ], style={'margin-top': '20px', 'margin-bottom': '60px'})
